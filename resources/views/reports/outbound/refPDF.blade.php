...
<body>
	 <img src="{{ public_path('images/company/'.$viewModel->getCompanySetting()[1]->value.'') }}" style="height: 50px;width: 100px;" />
		<span style="font-size:12pt;float:right;">Outbound Report Daily From: {{date_format(date_create($field->datefrom),'M d, Y')}} To: {{date_format(date_create($field->dateto),'M d, Y')}}</span>
              ...
              @if(count($viewModel->getOutboundLogs($date))>0 || count($viewModel->getOutboundsData($date))>0)
                ...
                @foreach($viewModel->getOutboundsData($date) as $ob)
                  <tr style="background: #F2F2F2">
                    <td colspan="1">{{$ob->refNo}}</td>
                    <td colspan="8">{{$ob->client->name}}</td>
                  </tr>
                  ...
                  @foreach($ob->outbounditems as $item)
                    <tr>
                      ...
                      <td>{{$ob->user->name}}<br/><small>{{date_format(date_create($ob->created_at),"M d, Y h:i a")}}</small></td>
                      <td>{{$ob->approvedby}}<br/><small>{{date_format(date_create($ob->ApprovedDateTime),"M d, Y h:i a")}}</small></td>
                    </tr>
