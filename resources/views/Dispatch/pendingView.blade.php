      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            ...
            <h1 class="m-0 text-dark"> <i class="fa fa-truck"></i> Delivery/Dispatch Ref # {{$viewModel->dispatchPending()->refNo}}</h1>
          </div><!-- /.col -->
              ...
              <div class="card-body">
                          ...
                          <div class="row">
                            <div class="col-md-4">
                                ...
                                <select class="form-control select2bs4" name="client" data-placeholder="Select Site" id="client">
                                ...
                                <div class="form-group">
                                  <label>Address</label>
                               <input type="text" class="form-control" name="address" id="address" value="{{$viewModel->dispatchPending()->address}}">
                                </div>
                            ...
                            <div class="col-md-4">
                               ...
                               <input type="text" class="form-control" name="control" value="{{$viewModel->dispatchPending()->controlno}}">
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                  <label>Date</label>
                                 <input type="date" class="form-control" name="date" value="{{$viewModel->dispatchPending()->date}}">
                                ...
                                <div class="form-group">
                                  <label>Terms</label>
                               <input type="text" class="form-control" name="terms"  value="{{$viewModel->dispatchPending()->terms}}">
                                </div>
                            ...
                            <div class="col-md-4">
                               ...
                               <input type="text" class="form-control" name="receivedby"  value="{{$viewModel->dispatchPending()->receivedby}}">
                                </div>
        ...
        <!-- /.row -->
          ...
          <div class="card-body">
              <div class="row">
                ...
                <div class="col-md-6">
                    <button id="addModal" disabled="true" data-toggle="modal" data-target="#addItemModal" type="button" class="btn btn-default float-right"><span class="fa fa-plus"></span> Manual Add</button>
                </div>
              </div>
              ...
              <table class="table table-hover">
                ...
                <tbody id="dispatchItems">
                    ...
                    @foreach($viewModel->dispatchPending()->dispatchitem as $diitem)
                        @if($diitem->itemlist_id == NULL)
                          <script>

                              itemManualArray.push("{{$diitem->item_id}}")
                          </script>
                           <tr id="item_{{$diitem->item_id}}">
                            <td><span style="font-weight:bold">{{$diitem->item->description}}</span><br/><small style="font-size:8pt">{{$diitem->item->productCode}}</small></td>
                            <td></td>
                            <td></td>
                            <td width="10%">
                            <input type="hidden" value="{{$diitem->item_id}}" name="itemID[]"/>
                            <input type="hidden" name="flag[]" value="0"/>
                            <input type="hidden" name="uom[]" value="{{$diitem->item->unitMeasurement}}"/>
                            <input type="hidden" value="{{$diitem->item_id}}" name="items[]"/>
                            <input id="itemqty_{{$diitem->item_id}}" name="quantity[]" type="number" class="form-control quantity" data-id="{{$diitem->item_id}}" value="{{$diitem->qty}}"></td>
                            <td>{{$diitem->uom}}</td>
                            <td><input type="text" class="form-control" name="remarks[]" value="{{$diitem->remarks}}" /></td>
                            <td><a href="javascript:void(0)" style="color:maroon;display: none;" class="_removeitem" data-id="{{$diitem->item_id}}"><span class="fa fa-times"></span></a></td>
                            </tr>
                        @else
                            <script>

                              itemarray.push(parseInt("{{$diitem->itemlist_id}}"))
                          </script>
                               <tr id="item{{$diitem->itemlist_id}}">
                                  <td><span style="font-weight:bold">{{$diitem->item->description}}</span><br/><small style="font-size:8pt">{{$diitem->item->productCode}}</small></td>
                                  <td>
                                    @if($diitem->itemlist->freeStorage == 0)
                                    {{$diitem->itemlist->barcode}}
                                    @endif
                                  </td>
                                  ...
                                  <input type="hidden" name="uom[]" value="{{$diitem->item->unitMeasurement}}"/>
                                  <input type="hidden" value="{{$diitem->itemlist->freeStorage}}" name="hasbarcode[]"/>
                                  <input type="hidden" value="{{$diitem->itemlist_id}}" name="items[]"/>
                                  <input id="itemqty{{$diitem->itemlist_id}}" name="quantity[]" type="number" class="form-control quantity" data-id="{{$diitem->itemlist_id}}" value="{{$diitem->qty}}"></td>
                                  <td>{{$diitem->item->unitMeasurement}}</td>
                                  <td><input type="text" class="form-control" value="{{$diitem->remarks}}" name="remarks[]"/></td>
                                  <td><a href="javascript:void(0)" style="color:maroon" class="removeitem" data-id="{{$diitem->itemlist_id}}"><span class="fa fa-times"></span></a></td>
                                  </tr>
            ...
            <div id="modifiedlogs" style="margin-top: 30px">
               @if($viewModel->dispatchPending()->LastModifiedBy != NULL)
              <span>Last Modified By: 
               
                {{$viewModel->dispatchPending()->LastModifiedBy}} // 
                {{date_format(date_create($viewModel->dispatchPending()->updated_at),'M d, Y h:i a')}}
             </span>
              @endif
...
function init(){
  ...
  $("select").val("{{$viewModel->dispatchPending()->client->id}}").trigger("change")
...
$(document).on('click','#updatebtn',function(){
    swal({
     ...
     function(isConfirm){
       ...
       if (isConfirm){
            $.ajax({
              url: '{{url("")}}/dispatch/update/{{$viewModel->dispatchPending()->id}}',
              type: 'PATCH',
              data: $('#createdispatchForm').serialize(),
              datatype: 'json',
            }).then((res)=>{
                       if(res.errors){
                          ...
                          if(res.message){
                            if(res.message == "success"){
                                    swal({
                                        title: "Delivery Succesfully Updated!!",
                                        //text: "You will not be able to recover this imaginary file!",
                                        type: "success",
                                        showCancelButton: false,
                                        showConfirmButton: false,
...
function confirmed(status){
 $.ajax({
    url: '{{url('')}}/dispatch/approved/{{$viewModel->dispatchPending()->id}}',
    type: 'PATCH',
    data: {
      '_token': $('input[name=_token]').val(),
      'status': status,
    ...
    datatype: 'json',
        ...
        if(res.message){
                    if(res.message == "success"){
                            ...
                            window.location.href = '{{url('')}}/dispatch/pending/list'
                    }else{
                               swal({
                                title: "Something Went Wrong!!",
                                text: "Please Try Again!",
          ...
          success:function(res){
            if(res.itemlist.length > 0){
                ...
                if(findid == -1){
                      ...
                      <input id="itemqty${res.itemlist[0].id}" name="quantity[]" type="number" class="form-control quantity" data-id="${res.itemlist[0].id}" value="1"></td>
