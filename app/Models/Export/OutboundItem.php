...
Class OutboundItem {
	...
	public function execute($viewModel,$field,$items){
		...
		header('Content-Disposition: attachment;filename=Outbound_Reports_Per_Items_'.$field->datefrom.'_TO_'.$field->dateto.'.xlsx');
 	...
 	$objPHPExcel->getActiveSheet()->calculateColumnWidths();
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("A4","Outbound Report Per Item From: ".date_format(date_create($field->datefrom),'M d, Y')." To: ".date_format(date_create($field->dateto),'M d, Y'));
		...
		foreach($items as $item){
			...
			$objPHPExcel->getActiveSheet()->mergeCells('A'.$row.':J'.$row);
		 	...
		 	if(count($viewModel->getOutbound_PerItem($item->id,$field->datefrom,$field->dateto)) > 0){
		 				...
		 				$objPHPExcel->getActiveSheet()->SetCellValue('A'.$row,date_format(date_create($obitem->unloadDate),'M d, Y'));
		 				$objPHPExcel->getActiveSheet()->SetCellValue('B'.$row,$obitem->outbound->refNo);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('C'.$row,$obitem->outbound->client->name);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('D'.$row,$obitem->itemlist->barcode);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('E'.$row,$obitem->itemlist->serialNumber);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('F'.$row,$obitem->itemlist->description);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('G'.$row,$obitem->quantity);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('H'.$row,$obitem->item->unitMeasurement);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('I'.$row,$obitem->outbound->user->name." ".date_format(date_create($obitem->outbound->created_at),"M d, Y h:i a"));
		 				$objPHPExcel->getActiveSheet()->SetCellValue('J'.$row,$obitem->outbound->approvedBy." ".date_format(date_create($obitem->outbound->ApprovedDateTime),"M d, Y h:i a"));
		 				$row++;
		 				$qty_total += $obitem->quantity;
		 				}
		 	...
		 	if(count($viewModel->getOutboundsItem_Logs_PerItem($item->id,$field->datefrom,$field->dateto))>0){
				 	...
				 	$row++;
		 				...
		 				foreach($viewModel->getOutboundsItem_Logs_PerItem($item->id,$field->datefrom,$field->dateto) as $itemlog){
					 				...
					 				$objPHPExcel->getActiveSheet()->SetCellValue('G'.$row,$itemlog->oldvalue - $itemlog->newvalue);
					 				...
					 				$row++;
					 				$qty_total += ($itemlog->oldvalue - $itemlog->newvalue);
