...
Class InboundItem {
	...
	public function execute($viewModel,$field,$items){
		...
		header('Content-Disposition: attachment;filename=Inbound_Reports_Per_Items_'.$field->datefrom.'_TO_'.$field->dateto.'.xlsx');
 	...
 	$objPHPExcel->getActiveSheet()->calculateColumnWidths();
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("A4","Inbound Report Per Item From: ".date_format(date_create($field->datefrom),'M d, Y')." To: ".date_format(date_create($field->dateto),'M d, Y'));
		...
		foreach($items as $item){
			...
			$objPHPExcel->getActiveSheet()->mergeCells('A'.$row.':J'.$row);
		 	...
		 	if(count($viewModel->getInbound_PerItem($item->id,$field->datefrom,$field->dateto)) > 0){
		 				...
		 				$objPHPExcel->getActiveSheet()->SetCellValue('B'.$row,"IB RefNo");
		 				...
		 				$objPHPExcel->getActiveSheet()->SetCellValue('A'.$row,date_format(date_create($ibitem->unloadDate),'M d, Y'));
		 				$objPHPExcel->getActiveSheet()->SetCellValue('B'.$row,$ibitem->inbound->refNo);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('C'.$row,$ibitem->inbound->client->name);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('D'.$row,$ibitem->itemlist->barcode);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('E'.$row,$ibitem->itemlist->serialNumber);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('F'.$row,$ibitem->itemlist->description);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('G'.$row,$ibitem->quantity);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('H'.$row,$ibitem->item->unitMeasurement);
		 				$objPHPExcel->getActiveSheet()->SetCellValue('I'.$row,$ibitem->inbound->user->name." ".date_format(date_create($ibitem->inbound->created_at),"M d, Y h:i a"));
		 				$objPHPExcel->getActiveSheet()->SetCellValue('J'.$row,$ibitem->inbound->approvedBy." ".date_format(date_create($ibitem->inbound->ApprovedDateTime),"M d, Y h:i a"));
		 				$row++;
		 				$qty_total += $ibitem->quantity;
		 				}
		 	...
		 	if(count($viewModel->getInboundsItem_Logs_PerItem($item->id,$field->datefrom,$field->dateto))>0){
				 	...
				 	$row++;
		 				...
		 				$objPHPExcel->getActiveSheet()->SetCellValue('G'.$row,"Qty");
		 				$objPHPExcel->getActiveSheet()->SetCellValue('H'.$row,"Unit");
		 				$objPHPExcel->getActiveSheet()->SetCellValue('I'.$row,"Modified By");
		 				$row++;
		 				foreach($viewModel->getInboundsItem_Logs_PerItem($item->id,$field->datefrom,$field->dateto) as $itemlog){

		 							$objPHPExcel->getActiveSheet()->SetCellValue('A'.$row,date_format(date_create($itemlog->created_at),'M d, Y'));
					 			
					 				$objPHPExcel->getActiveSheet()->SetCellValue('D'.$row,$itemlog->itemlist->barcode);
					 				$objPHPExcel->getActiveSheet()->SetCellValue('E'.$row,$itemlog->itemlist->serialNumber);
					 				$objPHPExcel->getActiveSheet()->SetCellValue('F'.$row,$itemlog->itemlist->description);
					 				$objPHPExcel->getActiveSheet()->SetCellValue('G'.$row,$itemlog->newvalue - $itemlog->oldvalue);
					 				$objPHPExcel->getActiveSheet()->SetCellValue('H'.$row,$itemlog->item->unitMeasurement);
					 				$objPHPExcel->getActiveSheet()->SetCellValue('I'.$row,$itemlog->modifiedby." ".date_format(date_create($itemlog->created_at),"M d, Y h:i a"));

					 				$row++;
					 				$qty_total += ($itemlog->newvalue - $itemlog->oldvalue);
