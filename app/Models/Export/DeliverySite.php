...
Class DeliverySite {
	...
	public function execute($viewModel,$field,$sites){
		...
		header('Content-Disposition: attachment;filename=Delivery_Reports_Per_Sites_'.$field->datefrom.'_TO_'.$field->dateto.'.xlsx');
 	...
 	$objPHPExcel->getActiveSheet()->calculateColumnWidths();
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("A4","Delivery Report Per Site From: ".date_format(date_create($field->datefrom),'M d, Y')." To: ".date_format(date_create($field->dateto),'M d, Y'));
		 		...
		 		if(count($viewModel->getDelivery_PerSite($site->id,$field->datefrom,$field->dateto))>0){
		 			...
		 			foreach($viewModel->getDelivery_PerSite($site->id,$field->datefrom,$field->dateto) as $dispatch){
		 				foreach($dispatch->item as $diitem){
		 					$objPHPExcel->getActiveSheet()->SetCellValue('A'.$row,date_format(date_create($dispatch->date),'M d, Y'));
				 			$objPHPExcel->getActiveSheet()->SetCellValue('B'.$row,$dispatch->refNo);
				 			$objPHPExcel->getActiveSheet()->SetCellValue('C'.$row,$diitem->description);
				 			$objPHPExcel->getActiveSheet()->SetCellValue('D'.$row,$diitem->productCode);

				 			if($diitem->pivot->itemlist_id != NULL){
				 				$dispatchdata = $viewModel->getItemlistInfo($dispatch->id,$diitem->pivot->itemlist_id);
				 				$objPHPExcel->getActiveSheet()->SetCellValue('E'.$row,$dispatchdata->itemlist->barcode);
					 			$objPHPExcel->getActiveSheet()->SetCellValue('F'.$row,$dispatchdata->itemlist->serialNumber);
					 			$objPHPExcel->getActiveSheet()->SetCellValue('G'.$row,$dispatchdata->itemlist->description);
				 			}
				 			
				 			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$row,$diitem->pivot->qty);
				 			$objPHPExcel->getActiveSheet()->SetCellValue('I'.$row,$diitem->pivot->uom);
				 			$objPHPExcel->getActiveSheet()->SetCellValue('J'.$row,$dispatch->user->name." ".date_format(date_create($dispatch->created_at),"M d, Y h:i a"));
				 			$objPHPExcel->getActiveSheet()->SetCellValue('K'.$row,$dispatch->approvedby." ".date_format(date_create($dispatch->ApprovedDateTime),"M d, Y h:i a"));
		 					$row++;
		 				}
