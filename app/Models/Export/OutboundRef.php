...
Class OutboundRef {
	...
	public function execute($viewModel,$field,$items){
 	...
 	$objPHPExcel->getActiveSheet()->calculateColumnWidths();
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("A4","Outbound Report Daily From: ".date_format(date_create($field->datefrom),'M d, Y')." To: ".date_format(date_create($field->dateto),'M d, Y'));


		$row = 6;

		foreach($viewModel->getDateRange($field->datefrom,$field->dateto) as $date){
			if(count($viewModel->getOutboundLogs($date))>0 || count($viewModel->getOutboundsData($date))>0){
			...
			foreach($viewModel->getOutboundsData($date) as $ob){
				$row++;
				$objPHPExcel->getActiveSheet()->SetCellValue("A".$row,$ob->refNo);
				$objPHPExcel->getActiveSheet()->SetCellValue("B".$row,$ob->client->name);
				...
				foreach($ob->outbounditems as $item){
					$row++;
					...
					$objPHPExcel->getActiveSheet()->SetCellValue('H'.$row,$ob->user->name." ".date_format(date_create($ob->created_at),"M d, Y h:i a"));
					$objPHPExcel->getActiveSheet()->SetCellValue('I'.$row,$ob->approvedBy." ".date_format(date_create($ob->ApprovedDateTime),"M d, Y h:i a"));
			...
			if(count($viewModel->getOutboundLogs($date))>0){
				$row++;
				$objPHPExcel->getActiveSheet()->mergeCells('A'.$row.':I'.$row);
				...
				$row++;
				foreach($viewModel->getOutboundLogs($date) as $log){
					...
					$objPHPExcel->getActiveSheet()->SetCellValue('E'.$row,($log->oldvalue - $log->newvalue));
